...
import { createButtonVotes } from "./likes.js";
...
const createCard = (comentario) => {
  ...
  // sectionCard.setAttribute("id", idComment);
  const contentLikes = createElement("aside");

  const contentHeader = createCommentHeader();
  const buttonLikes = createButtonVotes(
  ...
  const createYear = createElement("p", "text-title-comment");
  createYear.textContent = createdAt;

  const comment = createElement("section", "content-comment");
  const avatar = createUserAvatar(user);

  const textComment = createText(content);

  contentCard.append(contentLikes, comment);
  contentLikes.append(buttonLikes);
  comment.append(contentHeader, textComment);
  contentHeader.append(avatar, iconButton);
  avatar.append(createYear);
  return contentCard;
};
